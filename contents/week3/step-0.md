...
- `JSON.stringify()` :
자바스크립트의 객체 리터럴을 문자열(텍스트 형식)으로 바꿔준다. 서버로 데이터 형식을 보낼 때 필요하다.

- `JSON.parse()` :
는 서버에서 받은 데이터를 자바스크립트의 객체 리터럴로 바꿀 때 사용한다. 객체 리터럴로 변경하면 일반 자바스크립트 객체처럼 자유롭게 사용할 수 있다.
...
{% hint style="info" %}
**DSL(Domain-Specific Languages)** : 특정 분야에 최적화된 프로그래밍 언어를 말한다. JSX는 특정 환경, 특히 React에서만 사용할 수 있는 것은 아니지만, React를 다루기 쉽게 만드는 데 큰 비중을 차지한다. JSX에 대한 내용은 [JSX 페이지](../week2/jsx.md#undefined)를 참고하자.
{% endhint %}
